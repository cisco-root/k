<?php
//advanced by sudohardboy
//developer ramin
//our channel : @pmresannova and @fourteam
//-------------------------------------
//Set Your InforMation
$ToKeN = "678912904:AAH8A_4Rqrtz-EQRAVbBNsI0dj6VhcDU5IU";
$MyChannel = "@TelexBeta";
$Dev = 581956880;
$phone = '+989100000000';
$nea = 'Cisco';
...
//●●●●●●●●●●●●●●●●●●●
  ...
  SendMessage($from_id,"شما در بلاک لیست هستید❗️ من نمی توانم اجازه  ارسال پیام را به شما بدم⛔️");
  return;
 }
...
SendAction($chat_id, 'typing');
        ...
        'parse_mode' => "MarkDown",
 'chat_id'=>$chat_id,
 'text'=>"😉خوش اومدی به بات خود",
'reply_markup'=>json_encode([
      'keyboard'=>[
	  [['text'=>'ریست ربات⚪️']],
	  [['text'=>"حذف متن پروفایل❌"],['text'=>"بخش ارسال پیام📬"]],
	  [['text'=>"غیرفعال کردن دکمه ها❌"],['text'=>"فعال کردن دکمه ها✔️"]],
	  [['text'=>"غیرفعال کردن قفل کانال❎"],['text'=>"فعال بودن قفل کانال✅ "]],
	  [['text'=>"امار🌀"],['text'=>"یوزر ایدی ها امار🈁"]],
	  [['text'=>"خاموش کردن ربات🔴"],['text'=>"روشن کردن ربات🔵"]],
	  [['text'=>"بکاپ گیری ⏺"],['text'=>"مشخصات کاربرℹ️"]],
	  [['text'=>'راهنما*️⃣']],
...
'keyboard'=>[
	[['text'=>"پروفایل منℹ️"]],
	[['text'=>"ارسال شماره شما⚙️",'request_contact' => true],['text'=>"ارسال مکان شما🇮🇷",'request_location' => true]],
            ]
        ])
]);
}
...
'parse_mode'=>'MarkDown',
	...
	if ($from_id !== $Dev && !in_array($from_id,$banlist)){
		if($text !='/id' and $text !='/creator' and $text !='پروفایل منℹ️'){
			if($tc == "private"){
bot("forwardmessage",[
 'chat_id'=>$Dev,
...
elseif($text=="/id"){
  ...
  elseif($text=="/creator"){
      SendAction($chat_id, 'typing');
     SendMessage($chat_id,"این ربات توسط cisco طراحی و کد نویسی شده هست❗️
نویسنده سورس `cisco`");
}   
  elseif($text=="پروفایل منℹ️"){
...
//---------------------------------------------------------------------------------
    elseif($text=="یوزر ایدی ها امار🈁" && $from_id == $Dev){
		SendAction($chat_id,"typing");
...
//---------------------------------------------------------------------------------
    elseif($text=="بکاپ گیری ⏺" && $from_id == $Dev){
		SendAction($chat_id,"typing");
...
//---------------------------------------------------------------------------------
elseif($text=="راهنما*️⃣" && $from_id == $Dev){
	SendAction($chat_id, 'typing');
bot('sendmessage',[
'chat_id'=>$chat_id,
...
/ /setd `textpm`
/ /pro `tex` 
بقیه دستورات به صورت دکمه میباشد

Souce By `SUDOHARDBOY`
",
'parse_mode'=> 'MaRkDowN',
]);
}
//---------------------------------------------------------------------------------
elseif ($text == "روشن کردن ربات🔵" && $from_id == $Dev) {
	SendAction($chat_id, 'typing');
...
elseif ($text == "خاموش کردن ربات🔴" && $from_id == $Dev) {
	SendAction($chat_id, 'typing');
...
//---------------------------------------------------------------------------------
elseif ($text == "غیرفعال کردن دکمه ها❌" && $from_id == $Dev) {
	SendAction($chat_id, 'typing');
...
elseif ($text == "فعال کردن دکمه ها✔️" && $from_id == $Dev) {
	SendAction($chat_id, 'typing');
...
//---------------------------------------------------------------------------------
elseif ($text == "فعال بودن قفل کانال✅ " && $from_id == $Dev) {
	SendAction($chat_id, 'typing');
...
elseif ($text == "غیرفعال کردن قفل کانال❎" && $from_id == $Dev) {
	SendAction($chat_id, 'typing');
...
//---------------------------------------------------------------------------------
elseif($text == 'ارسال همگانی📨' && $from_id == $Dev){
	SendAction($chat_id, 'typing');
...
//---------------------------------------------------------------------------------
elseif($text == "فروارد همگانی📭" && $from_id == $Dev){
...
//---------------------------------------------------------------------------------
elseif($text == "پیام به کاربر📍" && $from_id == $Dev){
...
//---------------------------------------------------------------------------------
elseif($text=="ریست ربات⚪️" && $from_id == $Dev){
	SendAction($chat_id, 'typing');
	bot('SendMessage',[
'chat_id'=>$chat_id,
...
elseif ($text == "حذف متن پروفایل❌" && $from_id == $Dev) {
	SendAction($chat_id, 'typing');
  unlink("data/profile.txt");
bot('sendmessage',[
'chat_id'=>$chat_id,
...
elseif($text == "بخش ارسال پیام📬" && $from_id == $Dev){
bot('sendMessage',[
        'parse_mode' => "MarkDown",
 'chat_id'=>$chat_id,
 'text'=>"به بخش ارسال پیام خوش اومدید️",
'reply_markup'=>json_encode([
      'keyboard'=>[
	  	  [['text'=>'پیام به کاربر📍']],
...
//---------------------------------------------------------------------------------
elseif(strpos($text , '/sets')!== false && $from_id == $Dev){
	SendAction($chat_id, 'typing');
    $starttxt = str_replace('/sets',"",$text);
    if ($starttxt != ""){
         save("data/start.txt","$starttxt");
		SendMessage($chat_id,"متن شروع ربات تغییر یافت ✅\n⏺ متن جدید : \n$starttxt");
  }
}
elseif(strpos($text , '/set')!== false && $from_id == $Dev){
   SendAction($chat_id, 'typing');
   $donetxt = str_replace('/set',"",$text);
    if ($donetxt != ""){
         save("data/done.txt","$donetxt");
		SendMessage($chat_id,"متن پیش فرض ربات تغییر یافت ✅\n⏺ متن جدید : \n$donetxt");
  }
}
elseif(strpos($text , '/pro')!== false && $from_id == $Dev){
	SendAction($chat_id, 'typing');
    $proftxt = str_replace('/pro',"",$text);
    if ($proftxt != ""){
